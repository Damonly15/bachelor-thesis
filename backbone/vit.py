...
class VisionTransformer(MammothBackbone):
    ...
    def forward(self, x: torch.Tensor, AB: dict = {},  task_label=None, returnt='out'):
        """
        ...
        out = self.final_layer(feats, task_label)
...
def vit_base_patch16_224_prompt_prototype(pretrained=False, pretrain_type='in21k-ft-in1k', cpt=-1, **kwargs):
    ...
    if pretrain_type == 'in21k_old':
        model = create_vision_transformer('vit_base_patch16_224_in21k_fn_in1k_old', pretrained=pretrained, cpt=cpt, **dict(model_kwargs, **kwargs))
    elif pretrain_type == 'in21k':
        model = create_vision_transformer('vit_base_patch16_224.augreg_in21k', pretrained=pretrained, cpt=cpt, **dict(model_kwargs, **kwargs))
    else:
        model = create_vision_transformer('vit_base_patch16_224', pretrained=pretrained, cpt=cpt, **dict(model_kwargs, **kwargs))
    return model


def vit_backbone(num_classes, pretrained=True, cpt=-1, pretrain_type='in21k-ft-in1k'):
    return vit_base_patch16_224_prompt_prototype(pretrained=pretrained, pretrain_type=pretrain_type, cpt=cpt, num_classes=num_classes)
