...
class SequentialCIFAR100Shallow(ContinualDataset):
    ...
    def get_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), SequentialCIFAR100Shallow.TRANSFORM])
        return transform

    @staticmethod
    def get_backbone(args, model_compatibility):
        if (args.training_setting == 'task-il') and ('task-il' in model_compatibility):
            cpt = SequentialCIFAR100Shallow.N_CLASSES_PER_TASK #get backbone with different heads
        else:
            cpt = -1

        return resnet18(nclasses = SequentialCIFAR100Shallow.N_CLASSES, cpt=cpt, num_block=[1, 1, 1, 1])
    ...
    def get_normalization_transform():
        transform = transforms.Normalize(SequentialCIFAR100Shallow.MEAN, SequentialCIFAR100Shallow.STD)
        return transform

    @staticmethod
    def get_denormalization_transform():
        transform = DeNormalize(SequentialCIFAR100Shallow.MEAN, SequentialCIFAR100Shallow.STD)
        return transform
