...
class ErBounds22(ContinualModel):
    NAME = 'er_bounds22'
    ...
    def __init__(self, backbone, loss, args, transform):
        ...
        """
        super(ErBounds22, self).__init__(backbone, loss, args, transform)
        self.buffer = Buffer(self.args.buffer_size // 2)
        self.extra_buffer = Buffer(self.args.buffer_size // 2) #second buffer with different samples
        ...
        if self.args.training_setting == "class-il":
            if not self.buffer.is_empty():
                if epoch%2==0:
                    ...
                    buf_inputs, buf_labels, _ = self.extra_buffer.get_data(
                        self.args.minibatch_size, transform=self.transform, device=self.device)
            ...
            if not self.buffer.is_empty():
                if epoch%2==0:
                    ...
                    buf_inputs, buf_labels, buf_tasklabels = self.extra_buffer.get_data(
                        self.args.minibatch_size, transform=self.transform, device=self.device)
