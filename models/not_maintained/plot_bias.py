...
class PlotBias(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        ...
        """
        super(PlotBias, self).__init__(backbone, loss, args, transform)
        self.buffer = Buffer(self.args.buffer_size)
        self.buffer_refitting = Buffer(10)
    ...
    def end_task(self, dataset): #Changed this for the paper, it is from xder. It makes sure, that every class has the same amount of samples in the buffer.
        ...
        all_features, all_labels, all_tasklabels = get_features(self, dataset, 'train_dataset', self.current_task)
        features_head = LogisticRegression(max_iter=5000, C=10)
        features_head.fit(all_features.numpy(), all_labels.numpy())

        buffer_features, buffer_labels, _ = get_features(self, dataset, 'buffer', self.current_task)
