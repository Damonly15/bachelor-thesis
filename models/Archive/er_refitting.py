...
class ErRef(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        ...
        """
        super(ErRef, self).__init__(backbone, loss, args, transform)
    ...
    def begin_task(self, dataset):
        ...
        if self.current_task > 0:
            examples = self.args.buffer_size // self.current_task
            seen = 0

            input, label, _ = self.buffer.get_all_data()
            for _, c_label, c_input in dataset.train_loader:
                if seen > examples:
                    break
                input = torch.cat((input, c_input), dim=0)
                label = torch.cat((label, c_label), dim=0)

                seen += 32

            current_buffer = Buffer(2*self.args.buffer_size)
            current_buffer.add_data(
                        examples=input,
                        labels=label
                        )

            self.refitting_opt = Adam(self.net.classifier.parameters(), lr=0.001)

            for l in range(1000):
                buf_inputs, buf_labels = current_buffer.get_data(
                    64, transform=self.transform, device=self.device)
